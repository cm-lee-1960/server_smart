...
from management.models import morph_settings
# import logging
...
class Phone(models.Model):
    ...
    morph = models.CharField(max_length=100, null=True, blank=True)  # 모폴로지 재배정
    ...
    def update_phone(self, mdata):
        ...
        #     pass
        # else:
        morph_vul_list = morph_settings.objects.get(morph='취약지구').morph_vulzone.split(',') # 특정 단어를 포함하는지 확인하기 위해
        for morph_words in morph_vul_list:
            if morph_words in mdata.userInfo1:
                self.morph = '취약지구'
                break
            else:
                self.morph = morph_settings.objects.get(morph_startswith__startswith=mdata.userInfo2[0]).morph
