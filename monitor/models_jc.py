...
class PhoneGroup(models.Model):
    ...
    phone_no = models.BigIntegerField(default=0000000000, verbose_name='측정단말')
    ispId = models.CharField(max_length=10) # 한국:450 / KT:08, SKT:05, LGU+:60
    active = models.BooleanField(default=True)
    measure_type = models.CharField(max_length=5, default='etc', verbose_name='구분')
    type_count = models.IntegerField(null=True, default=0, verbose_name='구분 카운트')
    msg_sended = models.CharField(max_length=10000, null=True, default='X', verbose_name='메시지 전송 콜카운트')  # max_length 변경
    ...
    def current_count_check(self, phone):
        ...
        if phone.status == 'START':
            ...
            group_1 = PhoneGroup.objects.filter(userInfo1=self.userInfo1, measure_type=phone.phone_type).last()
            group_2 = PhoneGroup.objects.filter(userInfo1=self.userInfo1).exclude(measure_type=phone.phone_type).last()
            count_value = group_1.type_count
            if count_value in [1, 3, 10, 27, 37, 57] and group_2.type_count >= count_value :
                group_1.msg_sended=group_1.msg_sended + str(count_value) + ','#'번째 전송: ' + str(group_1.created_at) + ','
                group_1.save()
                result[1] = count_value
                result[0] = True
        return result
...
class Phone(models.Model):
    ...
    userInfo1 = models.CharField(max_length=100, null=True, blank=True, verbose_name='측정장소') # 입력된 주소정보
    ...
    def make_message(self):
        ...
        avg_downloadBandwidth = 0 ## 수정필
        avg_uploadBandwidth = 0 ## 수정필
        ccc = self.phoneGroup.current_count_check(self)
        if self.status in status and ccc[0]:
