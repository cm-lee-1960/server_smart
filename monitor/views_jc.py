...
def receive_json(request):
    ...
    measure_type = 'DL' if data['downloadBandwidth'] else 'UL' if data['uploadBandwidth'] else 'etc'  
    qs = PhoneGroup.objects.filter(userInfo1=data['userInfo1'], ispId=data['ispId'], active=True, measure_type=measure_type)#.order_by('-created_at')
    if qs.exists():
      phoneGroup = qs[0]
      phoneGroup.phone_no = data['phone_no']
      phoneGroup.measure_type = measure_type
      phoneGroup.type_count += 1
      phoneGroup.save()
    else:
      phoneGroup = PhoneGroup.objects.create(
                      ...
                      type_count=data['currentCount'],
                      #msg_sended='X'
                      )

    # 측정 단말기를 조회한다.
    #p = phoneGroup.phone_set.all().filter(phone_no=data['phone_no'], active=True)
    p = Phone.objects.filter(phone_no=data['phone_no'], active=True)
    if p.exists():
        phone = Phone.objects.create(
              phoneGroup = phoneGroup,
              userInfo1=data['userInfo1'],
              phone_type=measure_type,
              phone_no=data['phone_no'],
              networkId=data['networkId'],
              avg_downloadBandwidth=p.last().avg_downloadBandwidth,
              avg_uploadBandwidth=p.last().avg_uploadBandwidth,
              status='MEASURING',
              total_count=p.last().total_count+1,  # total_count = data['currentCount'] 로 변경 예정
