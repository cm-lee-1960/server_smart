...
from management.models import MessageConfig, PhoneInfo
...
def start_monitoring():
    ...
    if not holiday_check(dt.year, dt.month, dt.day) and MessageConfig.objects.filter(messageStatus='ALL').exists():
        msgcfg = MessageConfig.objects.filter(messageStatus='ALL')[0]
        if msgcfg.booleanValue is False:
            msgcfg.booleanValue = True
            msgcfg.save()


# ----------------------------------------------------------------------------------------------------------------------
# 매일 측정종료 후 처리해야 할 일들을 수행한다.
# 1) 측정단말 사전정보 내 파워온/오프 상태 (모두 오프로 설정)
# ----------------------------------------------------------------------------------------------------------------------
def daily_jobs_after_meas():

    # 측정단말 사정정보 내 파워온/오프를 오프로 재설정한다
    qs = PhoneInfo.objects.all()
    qs.update(power=False)